...
//$state=$_POST["state"];
									   $sn=0;
			$userSelect = "SELECT * FROM farmsales WHERE id='$id'";
...
<body class="theme-red">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <div class="card">
                        <div class="header">
                         ...
                         <div class="header">
                            <h2>SHOPPING CART </h2>
                           <font color="#FF0000" size="+3">MY WALLET BALANCE: &#8358;<?php echo number_format($Fetchedbal,2,'.',',') ?> </font>
                      <br> <font color="#FF0000" size="+3">TOTAL ORDER: &#8358;<?php echo number_format($total,2,'.',',') ?> </font>
					    </div>
                         
                           <div class="container">
    <div class="row">
        <div class="col-sm-9 col-md-9 col-md-offset-1">
		<form id="form_validation" method="post" action="cart-process.php" enctype="multipart/form-data">
    ...
    foreach ($_SESSION["cart_item"] as $item){
		?>
				...
				<td style="text-align:left;border-bottom:#F0F0F0 1px solid;"><?php echo "&#8358;".$item["price"]*$item["quantity"]; ?></td>
					<td style="text-align:left;border-bottom:#F0F0F0 1px solid;"><strong><?php echo $item["end_date"]; ?></strong></td>
					<td style="text-align:right;border-bottom:#F0F0F0 1px solid;"><?php echo "&#8358;".$item["other_market_price"]; ?></td>
				<td style="text-align:right;border-bottom:#F0F0F0 1px solid;"><?php echo "&#8358;".$item["amt_saved"]*$item["quantity"]; ?></td>
 ...
 $totalamount = 0;
				 ...
				 $query = "SELECT * FROM order_details WHERE product_code='$product_code'  ";
 ...
 $Fetchedqty = $r["quantity"];
        if( !$Fetchedqty ) { $Fetchedqty="&nbsp;"; }
 
 
$totalamount=$totalamount +  $Fetchedqty;
} **/
		
		        $total2=$item_qty;
				
				$selectName = "SELECT * FROM farmsales WHERE code = '$product_code'";
				$selectNameQ = mysqli_query($conStr,$selectName);
				$selectNameF = mysqli_fetch_array($selectNameQ,MYSQLI_ASSOC);
				$lastqty= $selectNameF['quantity'];

$_SESSION["total2"]=$total2;
$_SESSION["lastqty"]=$lastqty;
		?> 
				</tr>
                    ...
                    <tr>
					 <td>   </td>
                        ...
                        <td class="text-right"><h5><strong><?php echo "&#8358;".number_format($shipping,2,'.',',')."   <font color=red>Cost of Shipping has been added to Items bought</font>"; ?></strong></h5></td>
                    </tr>
                    <tr>
                          <td>   </td>
                        <td>   </td>
					...
					<input type="hidden" value="<?php echo $Fetchedbal  ?>" name="walletbal">
                    ...
                    <tr>
                        ...
                        <td>   </td>
                        <td>
                       <a href="farmsales_index.php"> <button type="button" class="btn btn-default">
