...
<body class="theme-red">
	  ...
	  // include("components/transactionBadge.php");
	   ?>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                    ...
                                    <tbody>
                                       <?php
									   $sn=0;
			$userSelect = "SELECT * FROM userinfo WHERE type='Admin' or type='Staff' ";
			...
			while($userSelectF = mysqli_fetch_array($userSelectQ,MYSQLI_ASSOC))
			{
			 ...
			 $userLoginId = $userSelectF['userLoginId'];
	...
	<?php
	  			...
	  			$selectUser = "SELECT * FROM  userlogin WHERE id='$userLoginId' ";
			 
			 
	  			$selectUserQ = mysqli_query($conStr,$selectUser);
				while($selectEventF = @mysqli_fetch_array($selectUserQ,MYSQLI_ASSOC))
				{
					$eventPostId = $selectEventF['id'];
					$userName = $selectEventF['userName'];
					$eMail = $selectEventF['eMail'];
				 	//$passWd = $selectEventF['password'];
					$type = $selectEventF['type'];
					 $newPassword = base64_decode($passWd);
...
?> 
        <tr>
			...
			<td> <img src="<?php echo $eventBanner ?>" class="img-responsive" align="left"/ width="150px" height="150px"></td>
		  <td><?php echo $sName." ".$fName." ".$lName ?></td>
		 ...
		 ?>
			...
			<!--<a href="" data-toggle="modal" data-target="<?php echo '#userlogin'.$eventPostId ?>">-->
			<a href="staff_update.php?id=<?php echo $eventPostId ?>"><span class="alert alert-primary" style="padding-top:5px; padding-bottom:5px; background-color:#000046; border-bottom:thick solid #000022; border-radius:2px;color:#fff;">Update</span></a>
 			 
			<div style="height:25px;"></div>
 			<a onClick="confirm('Are You Sure Your Want To Delete this User: <?php echo $userName ?> ')" href="staff_delete.php?type=<?php echo base64_encode('userlogin')?>&ie=<?php echo base64_encode($eventPostId) ?>&ca=<?php echo base64_encode(delete) ?>&it=<?php echo base64_encode($userType)?>" ><span class="alert" style="padding-top:5px; padding-bottom:5px; border-bottom:3px #990000  solid; border-radius:2px; background-color: #990000; color:#fff;">Delete</span></a>
			 <?php } ?>				
			</td>
            <!--<td></td>-->
        </tr>
