...
<body class="theme-red">
	   ...
	   ?>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                                    ...
                                    <tbody>
                                       <?php
									   $sn=0;
			$userSelect = "SELECT * FROM tlborder WHERE cust_id='$cust_id' AND productType!='Mile12Sales' AND productType!='Farmsales' order by id desc";
			...
			while($userSelectF = mysqli_fetch_array($userSelectQ,MYSQLI_ASSOC))
			{
				$eventPostId = $userSelectF ['id'];
				 $order_id = $userSelectF['order_id'];
				 $amount = $userSelectF['total_price'];
				 $status = $userSelectF['status'];
				 $cust_id = $userSelectF['cust_id'];
				$details = $userSelectF['productType'];
				$date = $userSelectF['date'];
				$postedby = $userSelectF['fullname'];
	...
	?>
        <tr>
		   ...
		   <td><?php echo $sn ?></td>
		      <td><?php echo $cust_id ?> <br><?php echo "<strong><font color=red>Wallet Bal: &#8358;".number_format($newBal)."</font></strong>" ?></td>
	...
	<div style="height:25px;"></div>
			<a onClick="confirm('Are You Sure Your Want To Confirm this Payment: <?php echo $amount ?> ')" href="confirmList.php?type=<?php echo base64_encode('tlborder')?>&ie=<?php echo base64_encode($eventPostId) ?>&ca=<?php echo base64_encode(confirm) ?>&it=<?php echo base64_encode($usertype)?>" ><span class="alert" style="padding-top:5px; padding-bottom:5px; border-bottom:3px #006633  solid; border-radius:2px; background-color: #006633; color:#fff;">Confirm</span></a>
<div style="height:25px;"></div>
			<a onClick="confirm('Are You Sure Your Want To Reject this Payment: <?php echo $amount ?> ')" href="rejectList.php?type=<?php echo base64_encode('tlborder')?>&ie=<?php echo base64_encode($eventPostId) ?>&ca=<?php echo base64_encode(confirm) ?>&it=<?php echo base64_encode($usertype)?>" ><span class="alert" style="padding-top:5px; padding-bottom:5px; border-bottom:3px #990000  solid; border-radius:2px; background-color: #CC0000; color:#fff;">Reject</span></a>				 
