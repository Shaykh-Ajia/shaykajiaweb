...
//$state=$_POST["state"];
			...
			while($selectEventF = mysqli_fetch_array($userSelectQ,MYSQLI_ASSOC))
			{
			 ...
			 $locationSelect = "SELECT * FROM userinfo WHERE cust_id='$cust_id'";
			$locationSelectQ = mysqli_query($conStr,$locationSelect);
			while($locationSelectF = mysqli_fetch_array($locationSelectQ,MYSQLI_ASSOC))
			{
				$eventPostId = $locationSelectF['id'];
					$eventSname = $locationSelectF['sName'];
					$phone = $locationSelectF['phone'];
					 
					 
			 
			 }
		 
	 $_SESSION["sName"]=$eventSname ;
...
<body class="theme-red">
    ...
    <div class="row">
		...
		<form id="form_validation" method="post" action="cart-process2.php" enctype="multipart/form-data">
